$logDir = Join-Path $env:TEMP "Install-NewApps"
if (-not (Test-Path $logDir)) { New-Item -Path $logDir -ItemType Directory -Force | Out-Null }
$transcriptPath = Join-Path $logDir "Install-NewApps_$(Get-Date -Format 'yyyyMMdd_HHmmss').log"
Start-Transcript -Path $transcriptPath -Force *> $null
...
Import-Module $PSScriptRoot\UDF\PSSomeEngineThings -WarningAction SilentlyContinue -Force
...
function Install-OfficeWithODT {
    ...
    Write-Verbose "ODT path: $odtPath"
    Write-Verbose "ODT exists: $(Test-Path $odtPath)"

    # Create configuration file path
    $configFileName = "office-config-$($Package.Id).xml"
    $configPath = Join-Path $TempPath $configFileName

    Write-Verbose "Creating ODT configuration file: $configPath"
    ...
    Write-Verbose "Product(s): $($products -join ', ')"
    Write-Verbose "Language: $language"
    Write-Verbose "Edition: ${officeEdition}-bit"
    Write-Verbose "Channel: $channel"

    # Display generated XML content for debugging
    if (Test-Path $configPath) {
        Write-Verbose "Generated XML:"
        Get-Content -Path $configPath | ForEach-Object { Write-Verbose "  $_" }
    }

    # Execute Office Deployment Tool
    Write-Verbose "Running Office Deployment Tool..."

    try {
        $arguments = @('/configure', $configPath)
        Write-Verbose "Command: $odtPath /configure `"$configPath`""
        ...
        if ($process.ExitCode -eq 0) {
            Write-Verbose "Office installation completed successfully"
        }
        else {
            # Check ODT log for details
            Write-Warning "ODT exit code: $($process.ExitCode)"
            Write-Verbose "Checking ODT logs..."

            # Look for ODT logs in common locations
            $logLocations = @(
                "$env:windir\Temp\OfficeDeploymentTool*.log",
                "$env:LOCALAPPDATA\Temp\OfficeDeploymentTool*.log",
                "$env:TEMP\OfficeDeploymentTool*.log"
            )
            foreach ($logPattern in $logLocations) {
                $logFiles = Get-ChildItem -Path $logPattern -ErrorAction SilentlyContinue | Sort-Object LastWriteTime -Descending | Select-Object -First 1
                if ($logFiles) {
                    Write-Verbose "ODT log found: $($logFiles.FullName)"
                    # Show last 20 lines of the log
                    Get-Content -Path $logFiles.FullName -Tail 20 | ForEach-Object { Write-Verbose "  $_" }
                    break
                }
            }
        ...
        switch ($Package.Source) {
            ...
            "url" {
                ...
                $Package.Installer = @{
                    ...
                    NestedInstallerType = $Package.UrlSource.NestedInstallerType
                }
...
try {
    ...
    try {
        ...
        # Determine appropriate User-Agent based on URL and source
        `$userAgent = if (`$installerUrl -match 'sourceforge\.net') {
            # SourceForge requires wget-like User-Agent for direct download
            "Wget"
        } elseif ('$packageSource' -eq 'url') {
            # URL source packages use a browser User-Agent to avoid 403 blocks
...
try {
    ...
    `$VerbosePreference = 'Continue'
    `$success = Install-MSStoreApp -Package `$storePackage *>&1 | ForEach-Object {
        if (`$_ -is [System.Management.Automation.VerboseRecord]) {
            Write-HostAndLog "  [Verbose] `$(`$_.Message)" -ForegroundColor DarkGray
        } elseif (`$_ -is [System.Management.Automation.WarningRecord]) {
            Write-HostAndLog "  [Warning] `$(`$_.Message)" -ForegroundColor Yellow
        } elseif (`$_ -is [string]) {
            Write-HostAndLog `$_
        } else {
            Write-HostAndLog "  `$_"
    ...
    if (`$success -eq `$true) {
        ...
        Write-HostAndLog "  Installation returned false (return value: `$success)" -ForegroundColor Red
        `$results += [pscustomobject]@{Name='$packageName'; Id='$packageId'; Source='$packageSource'; Success=`$false; RebootRequired=`$false}
    }
}
catch {
...
"@
            ...
            elseif ($packageSource -eq "odt") {
                # Build ODT configuration as PowerShell literals (avoids PS 5.1 ConvertTo-Json bugs)
                $odtConfig = $package.ODT
                $odtProducts = ($odtConfig.Products | ForEach-Object { "'$($_ -replace "'","''")'" }) -join ", "
                $odtLanguage = if ($odtConfig.Language) { $odtConfig.Language } else { "fr-fr" }
                $odtEdition = if ($odtConfig.OfficeClientEdition) { $odtConfig.OfficeClientEdition } else { "64" }
                $odtChannel = if ($odtConfig.Channel) { $odtConfig.Channel } else { "Current" }
                $odtDisplayLevel = if ($odtConfig.DisplayLevel) { $odtConfig.DisplayLevel } else { "None" }
                $odtAcceptEULA = if ($null -ne $odtConfig.AcceptEULA) { $odtConfig.AcceptEULA.ToString().ToLower() } else { "true" }
                $odtPinIcons = if ($null -ne $odtConfig.PinIconsToTaskbar) { $odtConfig.PinIconsToTaskbar.ToString().ToLower() } else { "false" }
                $odtAutoActivate = if ($null -ne $odtConfig.AutoActivate) { $odtConfig.AutoActivate.ToString().ToLower() } else { "true" }
                $odtExcludeApps = if ($odtConfig.ExcludeApps -and $odtConfig.ExcludeApps.Count -gt 0) {
                    "@(" + (($odtConfig.ExcludeApps | ForEach-Object { "'$($_ -replace "'","''")'" }) -join ", ") + ")"
                } else { "`$null" }
...
try {
    `$package = @{
        ...
        Source = 'odt'
        ODT = @{
            Products = @($odtProducts)
            Language = '$odtLanguage'
            OfficeClientEdition = '$odtEdition'
            Channel = '$odtChannel'
            DisplayLevel = '$odtDisplayLevel'
            AcceptEULA = `$$odtAcceptEULA
            PinIconsToTaskbar = `$$odtPinIcons
            AutoActivate = `$$odtAutoActivate
            ExcludeApps = $odtExcludeApps
        }
    }

    `$VerbosePreference = 'Continue'
    Install-OfficeWithODT -Package `$package *>&1 | ForEach-Object {
        if (`$_ -is [System.Management.Automation.VerboseRecord]) {
            Write-HostAndLog "  [Verbose] `$(`$_.Message)" -ForegroundColor DarkGray
        } elseif (`$_ -is [System.Management.Automation.WarningRecord]) {
            Write-HostAndLog "  [Warning] `$(`$_.Message)" -ForegroundColor Yellow
...
try {
    # Refresh PATH in case Chocolatey was just installed
    if (-not (Get-Command choco -ErrorAction SilentlyContinue)) {
...
function Get-SortedPackagesByDependencies {
    ...
    # Split machine packages into sub-groups to avoid reboot-pending issues:
    # 1. Standard packages (winget, msstore, odt, chocolatey, url)
    # 2. Windows Capabilities (RSAT, etc.) - may require reboot
    # 3. Windows Features (Hyper-V, Sandbox, etc.) - most likely to require reboot
    $rebootRiskSources = @("windowscapability", "windowsfeature")
    $machineStandard = @($machinePackages | Where-Object { $_.Source -notin $rebootRiskSources })
    $machineCapabilities = @($machinePackages | Where-Object { $_.Source -eq "windowscapability" })
    $machineFeatures = @($machinePackages | Where-Object { $_.Source -eq "windowsfeature" })

    Write-Verbose "Sorting machine standard packages ($($machineStandard.Count))..."
    $sortedStandard = Get-TopologicalSort -Items $machineStandard
    Write-Verbose "Sorting machine capabilities ($($machineCapabilities.Count))..."
    $sortedCapabilities = Get-TopologicalSort -Items $machineCapabilities
    Write-Verbose "Sorting machine features ($($machineFeatures.Count))..."
    $sortedFeatures = Get-TopologicalSort -Items $machineFeatures

    $sortedMachine = @()
    $sortedMachine += @($sortedStandard)
    $sortedMachine += @($sortedCapabilities)
    $sortedMachine += @($sortedFeatures)
